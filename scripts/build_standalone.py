...
Reads site/index.html + every site/assets/maps/hotter_than_france_*.png, base64-inlines
the maps into a `window.MAP_B64` lookup injected before </head>, and writes
site/hot-france-standalone.html — one portable file (open it anywhere, no server, no assets folder).

The page's renderDays() prefers window.MAP_B64[<date>_<theme>_<lang>] when present, and falls
...
def main() -> int:
    if not SRC.exists():
        ...
        return 1
    pngs = sorted(MAPS.glob("hotter_than_france_*.png"))
    if not pngs:
    ...
    entries = []
    for p in pngs:
        key = p.stem.replace("hotter_than_france_", "")  # <date>_<theme>_<lang>
